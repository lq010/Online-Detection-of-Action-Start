...
from Evaluation.utils import temporal_offset_2, interpolated_prec_rec

class UCFdetection(object):
    ...
    def _import_ground_truth(self, ground_truth_filename):
        ...
        for videoid, v in data.items():
            for GT_ins in v['annotations']:
                ...
                t_end_lst.append(GT_ins['timeStamp'][1])
    ...
    def evaluate(self):
        ...
        if self.verbose:
            ...
            for i in range(len(self.mAP)):
                ...
                print('\tmap: {:0.3f}'.format(self.mAP[i]))
...
def compute_average_precision_detection(ground_truth, prediction, tOffset_thresholds=np.linspace(1.0, 10.0, 10)):
    ...
    for idx, this_pred in prediction.iterrows():
        ...
        # print("######")
        # print(this_gt.values)
        # print('---')
        # print(this_pred)
        # exit()

        toff_arr, isOutOfBoundary = temporal_offset_2(this_pred['t-start'],
                               this_gt['t-start'].values,
                               this_gt['t-end'].values)
        ...
        for tidx, toff_thr in enumerate(tOffset_thresholds):
            for jdx in tOffset_sorted_idx:
                if toff_arr[jdx] > toff_thr or isOutOfBoundary[jdx]:
                    fp[tidx, idx] = 1
                    break
    ...
    for tidx in range(len(tOffset_thresholds)):      
        # print(precision_cumsum[tidx,:])
        # print(recall_cumsum[tidx,:])
