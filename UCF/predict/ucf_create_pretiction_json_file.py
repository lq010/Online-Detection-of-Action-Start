...
'''
GT_file = 'data/ucf_accident_test_ground_truth.json'
...
def get_result(rawPredictions, threshold_score):
    ...
    def print_attrs(name, obj):
        ...
        try:
            ...
            print('No GT for video <{}>, skiped.'.format(name))
            return
        for i in range (1,len(max_scores_indexes)):
            ...
            Sp = max_scores[i-1]
            try :
                Cp2 = int(max_scores_indexes[i-1])
            except:
                Cp2 = 0
            if Ct > 0 and  max_scores[i]>= threshold_score: # 0-> background, 1-20->actions
                if Cp != Ct or  Sp < threshold_score:
