...
from ucf_src.batch_generator_without_followup import batch_generator_AS_A_BG_2_1_1 as train_batch_generator
from ucf_src.batch_generator_without_followup import val_batch_generator
from schedules import onetenth10_4_8_12
...
def plot_history(history, result_dir):
    ...
    plt.ylabel('accuracy')
    plt.grid()
    plt.xticks([i for i in range(len(history.history['acc']))][::2])
...
def main(id): 
    ...
    # model_weight_filename = '/media/lq/C13E-1ED0/dataset/UCF_Crimes/results/adam_c3d_zz_10lr_pre_ABG/weights/weights.04-0.659.hdf5'
    ...
    N_val_samples = len(val_A_windows) *2 +len(val_AS_windows) #+ len(val_BG_windows)
...
# ##################################
    ...
    history = model.fit_generator(train_generator,
                                  steps_per_epoch =1,# N_train_iterations,
                                  epochs =1,# epochs,
                                  callbacks=[onetenth10_4_8_12(base_lr),csv_logger, tbCallBack, checkpointer],
                                  validation_data =  val_generator,
                                  validation_steps =1,# N_val_iterations,
                                  verbose=1)
