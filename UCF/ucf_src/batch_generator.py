...
def process_batch(windows, windows_length, img_path, train=True):
    N = len(windows)
    X_s = np.zeros((N,windows_length,120,160,3),dtype='float32') #start windows
    X_s_labels = np.zeros(N,dtype='int')
    X_f =  np.zeros((N,windows_length,120,160,3),dtype='float32') #follow up windows
    ...
    for i in range(len(windows)):
        ...
        if train:
            ...
            for j in range(windows_length):
                ...
                image_f = cv2.resize(image_f, (160, 120))
...
def val_batch_generator(AS_windows, A_windows, BG_windows, windows_length, batch_size, N_iterations, N_classes, img_path):
    N = (len(A_windows))   
    windows = AS_windows + A_windows + BG_windows[:N+batch_size]
