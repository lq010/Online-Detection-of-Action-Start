...
def process_batch(windows, windows_length, img_path, isTrain):
    N = len(windows)
    X_s = np.zeros((N,windows_length,120,160,3),dtype=np.float32) #start windows
    X_s_labels = np.zeros(N,dtype='int')
    # X_f =  np.zeros((N,windows_length,240,320,3),dtype=np.float32) #follow up windows
    ...
    for i in range(len(windows)): # len(windows) == batch size
        ...
        for j in range(windows_length):
            ...
            image_s = cv2.resize(image_s, (160,120))
...
def val_batch_generator_15_percentage(AS_windows, A_windows, BG_windows, windows_length, batch_size, N_iterations, N_classes, img_path):
    M = int(0.15*(len(A_windows)))   
    N = int(0.16*(len(BG_windows)))
    windows = AS_windows + A_windows[:M] + BG_windows[:N]
    while True:
        for i in range(N_iterations):
            # print(N_iterations)
            # print(len(windows))
...
def val_batch_generator_1_1(AS_windows, A_windows, BG_windows, windows_length, batch_size, N_iterations, N_classes, img_path):
    N = (len(A_windows))   
    windows = AS_windows + A_windows + BG_windows[:N]
    while True:
        for i in range(N_iterations):
